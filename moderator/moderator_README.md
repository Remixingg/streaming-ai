# Moderator Agent
![tag:innovationlab](https://img.shields.io/badge/innovationlab-3D8BD3)
[![link to source code](https://img.shields.io/badge/Source%20Code-E8ECF1?style=flat&logo=data%3Aimage%2Fsvg%2Bxml%3Bbase64%%2BCjwvc3ZnPgo%3D)](https://github.com/Remixingg/streaming-ai)
[![live](https://img.shields.io/badge/Live-8A2BE2?style=flat&logo=data%3Aimage%2Fsvg%2Bxml%3Bbase64%2CPHN2ZyB3aWR0aD0iMTAiIGhlaWdodD0iOCIgdmlld0JveD0iMCAwIDEwIDgiIGZpbGw9Im5vbmUiIHhtbG5zPSJodHRwOi8vd3d3LnczLm9yZy8yMDAwL3N2ZyI%%2BCg%3D%3D)](https://agentverse.ai/agents/details/agent1q07trs7qwy9avqjn89zpgxl09m4q905ezzd6k22spnxeu5wc8m70xa38xvl/profile)

This agent classifies a string in a streamer's chat on a live streaming platform as either playful banter (acceptable) or truly harmful content (unacceptable).

It uses LLM such as ASI:One and Gemini for the classification.

## Example input

```python
text = "You're so dumb lol"
```

## Example output

```python
ModerationResponse(
    is_inappropriate: true
)
```

## Usage Example

Copy and paste the following code into a new [Blank agent](https://agentverse.ai/agents/create/getting-started/blank-agent) for an example of how to interact with this agent.

```python
from uagents import Agent, Context, Model


class ModerationRequest(Model):
    text: str

class ModerationResponse(Model):
    is_inappropriate: bool


agent = Agent()


AI_AGENT_ADDRESS = "agent1q07trs7qwy9avqjn89zpgxl09m4q905ezzd6k22spnxeu5wc8m70xa38xvl"


text = "You're so dumb lol"


@agent.on_event("startup")
async def send_message(ctx: Context):
    await ctx.send(AI_AGENT_ADDRESS, ModerationRequest(text=text))
    ctx.logger.info(f"Sent text to Moderator Agent: '{text}")


@agent.on_message(ModerationResponse)
async def handle_response(ctx: Context, sender: str, msg: ModerationResponse):
    ctx.logger.info(f"Received response from {sender}")
    ctx.logger.info(f"Inappropriate? ==> {msg.is_inappropriate}")


if __name__ == "__main__":
    agent.run()
```

### Local Agent

1. Install the necessary packages:

   ```bash
   pip install uagents
   ```

2. To interact with this agent from a local agent instead, replace `agent = Agent()` in the above with:

   ```python
   agent = Agent(
       name="user",
       endpoint="http://localhost:8000/submit",
   )
   ```

3. Run the agent:
   ```bash
   python agent.py
   ```